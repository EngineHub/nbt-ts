...
export function stringify(
  ...
  const quoteChar =
    options.quote == "single" ? "'" : options.quote == "double" ? '"' : null;
  ...
  function stringify(tag: nbt.Tag, depth: number): string {
    ...
    if (tag instanceof nbt.Byte) return `${tag.value}b`;
      ...
      if (
        list.reduce((acc, x) => acc + x.length, 0) > breakLength ||
        list.some((text) => text.includes("\n"))
      ) {
        return `[\n${list
          .map((text) => spaces.repeat(depth) + text)
...
export function parse(text: string, options: ParseOptions = {}) {
  ...
  const unexpectedChar = (i?: number) => {
    if (i == null) i = index;
    return new Error(
      `Unexpected character ${text[index]} at position ${index}`
    );
  };
  ...
  function parse(): nbt.Tag {
    ...
    if (
      value != null &&
      (index == text.length || !unquotedRegExp.test(text[index]))
    ) {
      return value;
    }
